<template>
    <q-stepper v-model="step" vertical flat color="primary" animated>
        ...
        <q-step :name="2" title="安装皮肤MOD" icon="create_new_folder" :done="step > 2">
            请将MOD放置到你的MC安装目录的mods文件夹中，并启动客户端。
            <q-stepper-navigation>
                <q-btn @click="step++" color="primary" label="继续" />
                <q-btn flat @click="step--" color="primary" label="返回" class="q-ml-sm" />
            </q-stepper-navigation>
        </q-step>

        <q-step :name="3" title="将皮肤站添加至加载列表" icon="add_comment">
            请点击下方按钮下载此皮肤站的Extralist文件，然后将其放置到
            <pre class="q-d-inline">.minecraft/CustomSkinLoader/ExtraList</pre>文件夹中。
            <br>
            启动游戏后即可看到皮肤站的皮肤。

            <q-stepper-navigation>
                <q-btn @click="downloadExtraList" color="primary" label="下载 Extralist 文件" />
                <q-btn flat @click="step--" color="primary" label="返回" class="q-ml-sm" />
            </q-stepper-navigation>
        </q-step>
    </q-stepper>
